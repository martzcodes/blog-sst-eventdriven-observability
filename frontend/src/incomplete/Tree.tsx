...
export const Tree = ({ events = [] }: { events: any[] }) => {
  ...
  const processEvents = async () => {
    const fnEvents = events.filter((ev) => ev.meta?.fn).sort((a, b) => Number(a.sk) - Number(b.sk));
    
    const fnTree = fnEvents.reduce((p, c) => {
      const updatedObj = { ...p };
      if (Object.keys(p).includes(c.meta.fn)) {

      } else {
        updatedObj[c.meta.fn] = {
          sources: []
        }
      }
      return {...p}
